...
from dependency_analyzer import DependencyTreeAnalyzer, DependencyVisualizer
...
<body>
    ...
    <div class="section">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
            {% for lang, count in report_data.language_analysis.language_distribution.items()[:8] %}
            <div style="background: #f8f9fa; padding: 10px; border-radius: 5px; text-align: center;">
                <div style="font-weight: bold; color: #495057;">{{ lang or "Unknown" }}</div>
                <div style="color: #6c757d;">{{ count }} repos ({{ "%.1f"|format((count / report_data.statistics.total_analyzed) * 100) }}%)</div>
            </div>
            {% endfor %}
        </div>
    </div>
