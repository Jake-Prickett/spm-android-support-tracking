...
class EnhancedPackageSwiftParser:
    ...
    def _parse_dependency_declarations(self, deps_content: str) -> List[PackageDependency]:
        ...
        package_declarations = re.findall(r'\.package\([^)]*(?:\([^)]*\)[^)]*)*\)', deps_content)
    ...
    def _parse_single_dependency(self, declaration: str) -> Optional[PackageDependency]:
        ...
        try:
            ...
            name = name_match.group(1) if name_match else self._extract_name_from_url(url)
            ...
            self.logger.warning(f"Error parsing dependency declaration '{declaration}': {e}")
            return None
...
class DependencyTreeAnalyzer:
    ...
    def build_dependency_tree(self, refresh_cache: bool = False) -> Dict[str, DependencyNode]:
    ...
    def _build_package_cache(self):
        ...
        self.logger.info(f"Processing {len(repos)} repositories for dependency analysis")
        ...
        for repo in repos:
            ...
            if repo.package_swift_content:
                dependencies = self.parser.parse_package_swift(repo.package_swift_content)
            ...
            for dep in node.dependencies:
                if dep.resolved_owner and dep.resolved_repo:
                    ...
                    if dep_package_id in self._package_cache:
                        self._package_cache[dep_package_id].dependents.append(package_id)
    ...
    def _calculate_dependency_metrics(self):
        ...
        for package_id, node in self._package_cache.items():
            if package_id in self._dependency_graph:
                ...
                in_degree = self._dependency_graph.in_degree(package_id)  # How many depend on this
                out_degree = self._dependency_graph.out_degree(package_id)  # How many this depends on
    ...
    def get_dependency_tree_for_package(self, package_id: str, max_depth: int = 3) -> Dict[str, Any]:
    ...
    def _build_tree_recursive(self, package_id: str, max_depth: int, current_depth: int, visited: Set[str]) -> Dict[str, Any]:
        ...
        if package_id in self._dependency_graph:
            for dep_id in self._dependency_graph.successors(package_id):
                dep_tree = self._build_tree_recursive(dep_id, max_depth, current_depth + 1, visited.copy())
    ...
    def _calculate_tree_stats(self, tree: Dict[str, Any]) -> Dict[str, int]:
        ...
        def count_recursive(node: Dict[str, Any], depth: int):
            ...
            if node.get("package_id", "").count("/") == 1 and not node.get("has_package_swift"):
    ...
    def get_impact_analysis(self) -> Dict[str, Any]:
        ...
        for package_id, node in self._package_cache.items():
            if package_id in self._dependency_graph:
                ...
                direct_dependents = len(list(self._dependency_graph.predecessors(package_id)))
        ...
        return {
            ...
            "summary": {
                ...
                "high_impact_packages": len([p for p in impact_data if p["total_impact"] > 5]),
                "foundational_packages": len([p for p in impact_data if p["direct_dependents"] > 10])
            }
        }
    
    def _calculate_indirect_impact(self, package_id: str, visited: Set[str] = None) -> int:
        ...
        for dependent in self._dependency_graph.predecessors(package_id):
            indirect_count += 1 + self._calculate_indirect_impact(dependent, visited.copy())
...
class DependencyVisualizer:
    ...
    def generate_dependency_network_visualization(self, output_path: str, max_nodes: int = 100) -> str:
        """Generate interactive network visualization of dependencies."""
        self.logger.info(f"Generating dependency network visualization with max {max_nodes} nodes")
        ...
        for node in subgraph.nodes():
            ...
            package_data = next((p for p in top_packages if p["package_id"] == node), {})
    ...
    def generate_dependency_tree_html(self, package_id: str, output_path: str, max_depth: int = 3) -> str:
...
<body>
    <div class="container">
        ...
        def render_tree_node(node, depth):
            ...
            html += f'<div class="package-name">{node.get("package_id", "Unknown")}</div>'
...
def main():
    ...
    parser.add_argument('--output-dir', default='exports/dependencies', help='Output directory')
    parser.add_argument('--package', help='Specific package to analyze (owner/repo)')
    parser.add_argument('--max-nodes', type=int, default=100, help='Maximum nodes in network visualization')
    parser.add_argument('--max-depth', type=int, default=3, help='Maximum depth for tree analysis')
